...
function get_status()
{
	...
	foreach(glob($cron_dir.'/*'.Crontab\Config::$cron_suffix) as $cur_file)
	{
		...
		{
			$list_file[] = array('file'=>$file, 'description'=>$description, 'running'=>true, 'black'=>$black);
		}else{
			$list_file[] = array('file'=>$file, 'description'=>$description, 'running'=>false, 'black'=>$black);
			$error_file++;
		}
		$count_file++;
	}
