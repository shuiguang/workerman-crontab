...
class CrontabBrowser
{
    ...
    public function __construct($config = array())
    {
        ...
        if(isset($config['exec_file']))
        {
            $this->exec_file = $config['exec_file'];
        }else{
            if(basename($_SERVER['SCRIPT_FILENAME']) == $_SERVER['SCRIPT_FILENAME'])
            {
                $this->exec_file = isset($_SERVER['PWD']) ? $_SERVER['PWD'].'/'.$_SERVER['SCRIPT_FILENAME'] : $_SERVER['SCRIPT_FILENAME'];
            }else{
                $this->exec_file = $_SERVER['SCRIPT_FILENAME'];
            }
        }
    ...
    public function request_url($url)
    {
        if(isset($this->cookies['cookie_path']))
        {
            ...
            $cookie_path = sys_get_temp_dir().'/'.md5($this->exec_file).'.txt';
        }
